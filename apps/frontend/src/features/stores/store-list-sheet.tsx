...
const StoreListSheet = forwardRef<HTMLDivElement, StoreListSheetProps>(({ stores, onStoreClick }, ref) => {
    ...
    return (
        <div className="fixed bottom-[calc(var(--bottom-nav-height)+28px)] left-1/2 z-40 w-full max-w-[var(--app-max-width)] -translate-x-1/2 px-5">
            <div
                ref={ref}
                className={`w-full bg-white rounded-[32px] shadow-[0_18px_45px_rgba(15,23,42,0.12)] overflow-hidden transition-all duration-300 ease-in-out animate-fade-up ${sheetHeightClass}`}
            >
                <div
